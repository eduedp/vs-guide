...
module.exports = function VSGuide(mod) {
    ...
    let hooks = [];
    let insidemap = false;
    let bossInfo = undefined;
    let timers = {};
    let bossLoc = undefined;
    let flowerId = 999999999;
    let playerDebuffs = [];
    let bossHpWarningsNotified = [];
    // Lakan
    let shieldWarned = false;
    let timerNextMechanic = undefined;
    let lastNextAction = undefined;
    let lastInversionTime = undefined;
    let isReversed = false; // below 50% hp
    let inSoulWorld = false;
    ...
    function begoneRange() {
        for (let degree = 0; degree < 360; degree += 360 / 20) {
            spawnFlower(spawnLoc(degree, 250), 6000, collectionIds[0]);
        }
    }

    function laserStarNormal() {
        for (let i = 0; i < LakanLaserSafespots.length; i++) {
            spawnFlower(spawnLoc(LakanLaserSafespots[i], 450), 5000, collectionIds[1]);
        }
        for (let i = 0; i < LakanLaserNormalDangerOne.length; i++) {
            for (let radius = 100; radius < 1000; radius += 50) {
                spawnFlower(spawnLoc(LakanLaserNormalDangerOne[i], radius), 2500, collectionIds[0]);
            }
        }
    }
    function laserStarInverted() {
        ...
        for (let i = 0; i < LakanLaserInvertedDangerOne.length; i++) {
            for (let radius = 100; radius < 1000; radius += 50) {
                spawnFlower(spawnLoc(LakanLaserInvertedDangerOne[i], radius), 2500, collectionIds[0]);
            }
        }
    }
    ...
    function load() {
        if (!hooks.length) {
            hook('S_BOSS_GAGE_INFO', 3, (event) => {
                ...
                if (bossInfo.templateId === 3000) {
                    ...
                    if (Date.now() > (lastInversionTime + ShieldWarningTime) && !shieldWarned) {
                        const hint = (!inSoulWorld ? BossActions[3000][1401].msg : BossActions[3000][1402].msg);
            ...
            hook('S_DUNGEON_EVENT_MESSAGE', 2, (event) => {
                ...
                const msgId = parseInt(event.message.replace('@dungeon:', '')) % 10000;
                ...
                if (BossMessages[msgId]) {
                    ...
                    if (bossInfo.templateId === 3000) {
                        const lastNextActionBackup = lastNextAction;
                        lastNextAction = undefined;
    ...
    function unload() {
        if (hooks.length) {
            for (const h of hooks) mod.unhook(h);
